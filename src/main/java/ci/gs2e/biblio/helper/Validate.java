...
public class Validate {
	...
	*/
	...
	public static Validate RequiredValue(Map<String, Object> listAverifier) {
		for (Map.Entry<String, Object> item : listAverifier.entrySet()) {
			if(item.getValue() instanceof List<?>) {
				validate.setGood(((List<?>)item.getValue()) == null ? false : ((List<?>)item.getValue()).stream().anyMatch(v-> v != null && !v.toString().isEmpty()));
			}
			else {
				validate.setGood(item.getValue() == null ? false : !item.getValue().toString().isEmpty());
			}
	...
	public static <K, T> Response<T> validateObject(Request<K> request, Response<T> response, FunctionalError functionalError, Locale locale) throws Exception {
		boolean ostate = true;
	...
	public static <K, T> Response<T> validateList(Request<K> request, Response<T> response, FunctionalError functionalError, Locale locale) throws Exception {
		boolean ostate = true;
		response.setStatus(functionalError.REQUEST_FAIL("", locale));
		if (request == null) {
			response.setHasError(ostate);
	...
	public static <T, K> Response<T> validateDatasAggregation(Request<K> request, Response<T> response, FunctionalError functionalError, Locale locale) throws Exception {
		if (request == null) {
			response.setHasError(true);
			response.setStatus(functionalError.REQUEST_FAIL("la requete est vide", locale));
			return response;
		}
		if (request.getDatasAggregation() == null) {
			response.setHasError(true);
			response.setStatus(functionalError.REQUEST_FAIL("Aucun objet `datasAggregation` dans la requête.", locale));
			return response;
		}

		if (request.getDatasAggregation().isEmpty()) {
			response.setHasError(true);
			response.setStatus(functionalError.REQUEST_FAIL("la liste `datasAggregation` fournie est vide", locale));
			return response;
		}
