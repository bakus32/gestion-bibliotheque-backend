...
import ci.gs2e.biblio.dao.repository.customize._FonctionnaliteRepository;
...
public interface FonctionnaliteRepository extends JpaRepository<Fonctionnalite, Integer>, _FonctionnaliteRepository {
	 ...
	 *         no Fonctionnalite is found, this method returns null.
	 */
	@Query("select e from Fonctionnalite e where e.parentId = :parentId and e.isDeleted = :isDeleted")
	...
	default String generateCriteria(FonctionnaliteDto dto, HashMap<String, Object> param, Integer index,  Locale locale) throws Exception{
		...
		if (dto != null) {
			...
			if (dto.getParentId() != null || Utilities.searchParamIsNotEmpty(dto.getParentIdParam())) {
				listOfQuery.add(CriteriaUtils.generateCriteria("parentId", dto.getParentId(), "e.parentId", "Integer", dto.getParentIdParam(), param, index, locale));
			}
