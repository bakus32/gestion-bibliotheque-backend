...
public interface _UsersRepository {
	default List<String> _generateCriteria(UsersDto dto, HashMap<String, Object> param, Integer index, Locale locale) throws Exception {
	 ...
	 @Query("select e from Users e where e.login = :login and e.passwd = :passwd and e.isDeleted = :isDeleted")
	 Users findByLoginAndPassword(@Param("login") String login,@Param("passwd") String passwd,@Param("isDeleted") Boolean isDeleted);
}
